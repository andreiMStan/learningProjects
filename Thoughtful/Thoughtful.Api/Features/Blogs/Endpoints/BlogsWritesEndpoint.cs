...
namespace Thoughtful.Api.Features.Blogs.Endpoints
{
				public class BlogsWritesEndpoint:IEndpoint
				{
								...
								{
												endpoints.MapPost("/api/blogs",async(BlogCreate dto)=>await CreateBlog(dto)).WithName("Create blog").WithDisplayName("Blogs")
																.Produces<BlogCreate>()
																.Produces(500);

												endpoints.MapPost("/api/blogs/{blogId}/contributors", async (int blogId, ContributorAdd contributor) => await
												AddBlogContributor(blogId,contributor)).WithName("AddContributor")
												.WithDisplayName("Blogs")
												.Produces(204)
												.Produces(500);

												endpoints.MapDelete("/api/blogs/{blogId}/contributors/{contributorId}",async(int blogId, int contributorId)=>await RemoveContributor(blogId,contributorId))
												...
												private async Task<IResult> RemoveContributor(int blogId,int contributorId)
								{
												...
												public record BlogCreate(string Name, string Description, int AuthorId);
